...
void drawEnv(float size)
{
    ...
    //glActiveTexture(GL_TEXTURE0 + MAP);
    glDisable(GL_TEXTURE_CUBE_MAP);  //天空的纹理坐标手工设置
}

void drawEn(float size)
{
    ...
    glTexCoord2i(0, 0); glVertex3f(-size, -size,  size);
    glTexCoord2i(1, 0); glVertex3f( size, -size,  size);
    glTexCoord2i(1, 1); glVertex3f( size,  size,  size);
    glTexCoord2i(0, 1); glVertex3f(-size,  size,  size);
...
void drawBunny()
{
    ...
    char FileName[128];
    sprintf(FileName, "%skatanas.obj", root);
...
int main(int argc, char *argv[])
{
    ...
    //programs[1] = setupShaders("Phong");
    //programs[2] = setupShaders("EpicUnreal");
    sprintf(path, "%sCook-Torrorence", root);
    programs[3] = setupShaders(path);
    ...
    glEndList();
    glNewList(SHAPE_BUNNY, GL_COMPILE);
    drawBunny();
    glEndList();
    ...
    TwAddVarRW(bar, "Roughness", TW_TYPE_FLOAT, &roughness, " label='Roughness' min=0 max=2 step=0.01 keyIncr='+' keyDecr='-' ");
    
    {
        TwEnumVal shaders[NUM_SHADERS] = { {SHADER_PHONG, "Phong"}, {SHADER_UNREALENGINE, "UnrealEngine"}, {SHADER_COOKTORRORENCE, "CookTorrorence"} };
    ...
    // (before adding an enum variable, its enum type must be declared to AntTweakBar as follow)
    {
        ...
        TwEnumVal shapeEV[NUM_SHAPES] = { {SHAPE_TEAPOT, "Teapot"}, {SHAPE_TORUS, "Torus"}, {SHAPE_BUNNY, "Bunny"} };
